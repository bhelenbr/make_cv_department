...
from merge_df import update_keep_old
...
# Percent effort of 0 (or blank) can't be used to back out the total; leave it blank
pct = pd.to_numeric(df["PCT"], errors="coerce")
df["Total Cost"] = (df["Allocated Amt"]/pct.where(pct > 0))*100.
...
for faculty_dir in faculty_path.iterdir():
	...
	if FacultyName.find(",") > -1:
		...
		if entries.empty:
			print(f"Adding grants to {FacultyName}: No entries")
			continue
		...
		if destination.is_file():
			...
			if "Proposal_ID" in df_old.columns:
				df_old = df_old.set_index("Proposal_ID",drop=True)
				# Update rows from the report, keep edited Sponsor/Title, and keep rows no longer in the report
				merged = update_keep_old(entries,df_old,cols_from_old=["Sponsor", "Title"])
			else:
				merged = entries
