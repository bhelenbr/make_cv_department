...
file = "personal awards data.xlsx"
folder = "Awards"
...
compilation.write("Title,Type,Year,FacultyName\n")
compilation.close()
...
for FacultyName in os.listdir(file_source): # For each faculty member
	if FacultyName[0].isalnum(): # gets rid of hidden files generated, allows for only faculty names left
		print("Personal Awards: " +FacultyName)
		try:
			...
			sheet.to_csv(open(tempcsv, 'a+'),columns=["Title","Type","Year","FacultyName"],index=False,header=False)
